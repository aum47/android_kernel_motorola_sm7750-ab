...
static int ufshid_get_analyze_and_issue_execute(struct ufshid_dev *hid)
{
	...
	if (ufshid_read_attr(hid, QUERY_ATTR_IDN_HID_FRAG_STATUS_MICRON_UFSV4, &frag_level))
		return -EINVAL;
	//get micron ufs hid execution progress
	if (ufshid_read_attr(hid, QUERY_ATTR_IDN_HID_PROGRESS_MICRON_UFSV4, &attr_val))
		return -EINVAL;
...
static inline void ufshid_set_lba_param(struct ufshid_dev *hid, int lun,
...
void ufshid_prep_fn(struct ufsf_feature *ufsf, struct ufshcd_lrb *lrbp)
{
	...
	if (ufshid_check_file_info_buf(hid, page_address(page), len)) {
					  ...
					  const char *buf, size_t count)
{
	...
	if (ret) {
		INFO_MSG("Changing trigger val %d is fail (%ld)", val, ret);
		return ret;
	}
...
static ssize_t ufshid_sysfs_show_l2p_frag_lvl(struct ufshid_dev *hid, char *buf)
{
	...
	INFO_MSG("l2p_defrag_lvl %u", attr_val);
...
static ssize_t ufshid_sysfs_show_l2p_param_print(struct ufshid_dev *hid,
						 char *buf)
{
	ufsf_print_buf(hid->hid_req.buf, hid->hid_req.buf_size);

	return snprintf(buf, PAGE_SIZE, "Check Dmesg\n");
}
...
static ssize_t do_ufshid_sysfs_show_color_samsung(struct ufshid_dev *hid, char *buf)
{
	...
	if (ret) {
		...
		return snprintf(buf, PAGE_SIZE, "UNKNOWN: Error\n");
	}
...
static ssize_t do_ufshid_sysfs_show_color_micron(struct ufshid_dev *hid, char *buf)
{
	...
	if (ufshid_read_attr(hid, QUERY_ATTR_IDN_HID_FRAG_STATUS_MICRON_UFSV4, &attr_val))
		return -EINVAL;
	frag_level = attr_val;
						...
						const char *buf, size_t count)
{
	...
	INFO_MSG("HID_block_suspend %d", val);
